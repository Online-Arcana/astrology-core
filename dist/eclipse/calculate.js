...
export const eclipseProfile = "western_eclipses/1.1.0";
const unavailable = (reason) => ({
    status: reason === "outside_supported_range" ? "unsupported" : "unavailable",
    value: null,
    reason,
});
const resultStatus = (time) => {
    if (time.resolution.status === "exact")
        return { status: "exact", reason: "none" };
...
const noTimeReason = (time) => time.resolution.reason === "none" ? "birth_time_unknown" : time.resolution.reason;
const distance = (a, b) => {
    const delta = Math.abs(normaliseDegrees(a) - normaliseDegrees(b));
    return Math.min(delta, 360 - delta);
};
const longitude = (astronomy, id, jde) => normaliseDegrees(astronomy.sample(id, jde).eclipticLongitudeRadians * 180 / Math.PI);
const facts = (event, astronomy, lunarOrbit) => {
...
const scan = (birthJde, eclipses) => {
    const year = eclipses.decimalYear(birthJde);
    const events = new Map();
    const kinds = ["solar", "lunar"];
...
const prenatal = (kind, birthJde, events) => {
    const candidates = events.filter((event) => event.kind === kind && event.julianEphemerisDay < birthJde);
    return candidates.at(-1) ?? null;
};
const atBirth = (birthJde, events) => {
    const candidates = events
        .filter((event) => Math.abs(event.julianEphemerisDay - birthJde) <= event.activeHalfDurationDays)
        .sort((a, b) => Math.abs(a.julianEphemerisDay - birthJde) - Math.abs(b.julianEphemerisDay - birthJde));
    return candidates[0] ?? null;
};
const prenatalValue = (event, birthJde, astronomy, lunarOrbit, eclipses, zodiac, ayanamsha) => {
    const eventFacts = facts(event, astronomy, lunarOrbit);
    const longitudeDegrees = zodiac === "tropical"
        ? eventFacts.longitudeDegrees
        : eventFacts.longitudeDegrees - ayanamshaDegrees(event.julianEphemerisDay, ayanamsha ?? "lahiri");
    return {
        kind: event.kind,
        type: event.type,
...
const natalValue = (event, birthJde, astronomy, lunarOrbit, eclipses) => {
    const eventFacts = facts(event, astronomy, lunarOrbit);
    return {
        kind: event.kind,
        type: event.type,
...
export const calculateEclipses = (input) => {
    ...
    return {
        ...
        prenatalSolar: solar
            ? {
                status: state.status,
                value: prenatalValue(solar, birthJde, input.astronomy, input.lunarOrbit, input.eclipses, input.zodiac, input.ayanamsha),
                reason: state.reason,
            }
            : unavailable("insufficient_data"),
        prenatalLunar: lunar
            ? {
                status: state.status,
                value: prenatalValue(lunar, birthJde, input.astronomy, input.lunarOrbit, input.eclipses, input.zodiac, input.ayanamsha),
                reason: state.reason,
            }
            : unavailable("insufficient_data"),
    };
