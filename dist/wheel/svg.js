...
const image = async (assets, path, fallback, x, y, size, rotation = 0, modifier) => {
    if (assets === undefined)
        return `<text x="${x.toFixed(3)}" y="${(y + size * .32).toFixed(3)}" text-anchor="middle" font-size="${size}">${esc(fallback)}</text>`;
...
const line = (longitude, from, to, asc, cls) => {
...
export const renderSvg = async (data, options = {}) => {
    ...
    if (timed && house.status !== "unavailable")
        for (const h of Object.values(house.houses)) {
