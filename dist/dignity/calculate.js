...
import { boundsProfile, chaldeanFaceSequence, dignityProfile, egyptianBounds, exaltationRulers, facesProfile, fallSigns, detrimentSigns, signElements, triplicityProfile, triplicityRules, } from "./catalogue.js";
const classical = new Set(["sun", "moon", "mercury", "venus", "mars", "jupiter", "saturn"]);
const includes = (values, value) => values?.includes(value) ?? false;
export const boundRuler = (position) => {
    const segment = egyptianBounds[position.sign].find(({ start, end }) => position.degreeWithinSign >= start && position.degreeWithinSign < end);
    if (!segment)
        throw new Error(`Egyptian bounds do not cover ${position.sign} ${position.degreeWithinSign}`);
    return segment.ruler;
};
export const faceRuler = (position) => {
    ...
    return chaldeanFaceSequence[decanIndex % chaldeanFaceSequence.length];
};
const state = (planet, position, sect) => {
    ...
    if (domicile)
        ruleRefs.push(`${dignityProfile}#domicile`);
    if (exalted)
        ruleRefs.push(`${dignityProfile}#exaltation`);
    if (detriment)
        ruleRefs.push(`${dignityProfile}#detriment`);
    if (fallen)
        ruleRefs.push(`${dignityProfile}#fall`);
    if (primaryTriplicity)
        ruleRefs.push(`${triplicityProfile}#${signElements[position.sign]}.${sect}`);
    if (participatingTriplicity)
        ruleRefs.push(`${triplicityProfile}#${signElements[position.sign]}.participating`);
    if (boundRulerState)
        ruleRefs.push(`${boundsProfile}#${position.sign}.${bound}`);
    if (faceRulerState)
        ruleRefs.push(`${facesProfile}#decan-${Math.floor(position.longitudeDegrees / 10) + 1}`);
    if (!positive)
        ruleRefs.push(`${dignityProfile}#peregrine`);
    return {
        traditionalRuler,
        modernRuler,
        domicile,
...
export const calculateDignity = (planet, position, sect) => {
    if (position.value === null)
        return unavailable(position.reason);
    if (sect.value === null) {
        return {
            status: "bounded",
