...
const scale = (value, factor) => ({ x: value.x * factor, y: value.y * factor, z: value.z * factor });
const cross = (a, b) => ({
    x: a.y * b.z - a.z * b.y,
    y: a.z * b.x - a.x * b.z,
    z: a.x * b.y - a.y * b.x,
...
const longitude = (value) => normaliseDegrees(Math.atan2(value.y, value.x) * radiansToDegrees);
const polynomial = (t, coefficients) => {
    let value = 0;
...
const meanApogee = (jde) => {
    const t = (jde - 2_451_545) / 36_525;
    const perigee = polynomial(t, [83.3532465, 4069.0137287, -0.01032, -1 / 80_053, 1 / 18_999_000]);
    return perigee + 180;
};
const osculating = (moon, jde) => {
    const step = 0.001;
    const current = vector(moon.position(jde));
    const velocity = scale(subtract(vector(moon.position(jde + step)), vector(moon.position(jde - step))), 1 / (2 * step));
    const angularMomentum = cross(current, velocity);
    if (magnitude(angularMomentum) === 0)
        throw new Error("Lunar state has no angular momentum");
    const eccentricity = subtract(scale(cross(velocity, angularMomentum), 1 / earthMoonMuKm3PerDay2), scale(current, 1 / magnitude(current)));
    if (magnitude(eccentricity) < 1e-9)
        throw new Error("Lunar osculating apogee is undefined");
    return {
...
const longitudes = (moon, nutation, jde) => {
...
class AstronomiaLunarOrbit {
    #moon;
    #nutation;
    constructor(moon, nutation) {
        this.#moon = moon;
...
export const loadLunarOrbit = async () => {
...
//# sourceMappingURL=lunarOrbit.js.map